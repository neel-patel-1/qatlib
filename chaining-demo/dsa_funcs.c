...
const unsigned long crc32_table[8][256] = {
{
...
void prepare_memcpy_task(
  ...
  tsk->dflags = IDXD_OP_FLAG_CRAV | IDXD_OP_FLAG_RCR | IDXD_OP_FLAG_BOF | IDXD_OP_FLAG_CC;
  acctest_prep_desc_common(tsk->desc, tsk->opcode, (uint64_t)(tsk->dst1),
				 (uint64_t)(tsk->src1), tsk->xfer_size, tsk->dflags);
...
CpaStatus dsaCrcGenCompareWithSw(Cpa8U *buf, Cpa32U buf_size){
  ...
  tsk_node = dsa->multi_task_node;
  int dsa_crc = tsk_node->tsk->comp->crc_val;
  struct task *tsk = tsk_node->tsk;
  int seed_val = tsk->crc_seed;
  uint8_t *inp = tsk->src1;
  uint32_t size = tsk->xfer_size;
