...
# Initialize decomp_and_hash_logs
mkdir -p decomp_and_hash_logs
for q in "${QUERY_SIZES[@]}"; do
    echo -n > decomp_and_hash_logs/core_${CORE}_querysize_${q}.log
done
...
for q in "${QUERY_SIZES[@]}"; do
    ...
    iters=${ITERATIONS[$j]}
    taskset -c ${CORE} sudo ./decomp_and_hash \
        -d $DEVID \
        -q $q \
        -t ${REQUESTS[$qidx]} \
        -i $iters >> decomp_and_hash_logs/core_${CORE}_querysize_${q}.log &
    wait
    qidx=$((qidx+1))
done